...
data_dir = os.path.join(current_dir, "./test_circuit_55/32_3dconv55.csv")
...
def execute(C):
    ...
    for i in range(d):
        ...
        for s in range(k[i] + 2 * (k[i + 1] - num_copy[i]) + 1):
            ...
            r = verifier_inst.partial_sumcheck_check_parallel(i, s, prover_msg)
            if DEBUG_INFO:
                if s != 0:
                    print(
...
test_circuit = circuit.createCircuit(
    data_dir, [5, 5, 6, 7, 8, 9, 10, 11, 12], mpz(prime)
)
